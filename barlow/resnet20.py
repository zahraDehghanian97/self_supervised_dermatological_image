...
class ResNet:
    ...
    def learner(self, x, n_blocks):
        ...
        x = self.residual_group(x, 16, n_blocks, strides=(1, 1), n=4)
        ...
        x = self.residual_group(x, 64, n_blocks, n=2)

        # Third Residual Block Group of 64 filters (Stage 3)
        # Double (2X) the size of filters and reduce feature maps by 75% (strides=2) to fit the next Residual Group
        x = self.residual_group(x, 128, n_blocks, n=2)
        return x

    def residual_group(self, x, n_filters, n_blocks, strides=(2, 2), n=2):
        ...
        x = self.projection_block(x, n_filters, strides=strides, n=n)
    ...
    def projection_block(self, x, n_filters, strides=(2, 2), n=2):
    ...
    def projection_head(self, x, hidden_dim=128):
    ...
    def get_network(self, in_shape, n=2, hidden_dim=128, use_pred=False, return_before_head=True):
        depth = n * 9 + 2
        ...
        # The learner
        x = self.learner(x, n_blocks)

        # Projections
        trunk_output = GlobalAvgPool2D()(x)
        projection_outputs = self.projection_head(trunk_output, hidden_dim=hidden_dim)
        ...
        # Predictions
        if use_pred:
            prediction_outputs = self.prediction_head(projection_outputs)
            if return_before_head:
